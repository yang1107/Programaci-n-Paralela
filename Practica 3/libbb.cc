...
void Equilibrado_Carga(tPila *pila, bool *fin){
  MPI_Status status;
  int tipoMensaje, count, origen, fuente;

  if(pila->vacia()){   // el proceso no tiene trabajo
    //envia peticion de trabajo al proceso siguiente id+1%p
    ...
    while(pila->vacia() && !*fin){
      ...
      switch(tipoMensaje){
        case PETICION: // si es mensaje de peticion de trabajo
          //recibir mensaje de peticion de trabajo
          MPI_Recv(&origen, 1, MPI_INT, anterior, PETICION, comunicadorCarga, &status);
          if(origen == rank){ //peticion devuelta
            //reenviar peticion de trabajo al siguiente proceso
            MPI_Send(&origen, 1, MPI_INT, siguiente, PETICION, comunicadorCarga);
            //deteccion de fin
          }
          else{ // peticion de otro proceso la retransmite al siguiente
            //pasar peticion de trabajo al siguiente proceso
            MPI_Send(&origen, 1, MPI_INT, siguiente, PETICION, comunicadorCarga);
          }
        break;
        case NODOS: //resultado de una peticion de trabajo
          //recibir nodos del proceso donante y almacenar en la pila
          fuente = status.MPI_SOURCE;
          MPI_Get_count(&status, MPI_INT, &count);
          MPI_Recv(pila->nodos, count, MPI_INT, fuente, NODOS, comunicadorCarga, &status);
          pila->tope = count;
        break;
      }
    }
  ...
  if(!*fin){ // el proceso tiene nodos para trabajar
    ...
    while (flag>0) { // hay mensaje
      //recibir mensaje de peticion de trabajo
      MPI_Recv(&origen, 1, MPI_INT, anterior, PETICION, comunicadorCarga, &status);

      if(pila->tamanio()>1){ //hay suficiente nodos en la pila
        tPila pila_aux;
        pila->divide(pila_aux);
        //enviar nodo al proceso solicitante
        MPI_Send(pila_aux.nodos, pila_aux.tope, MPI_INT, origen, NODOS, comunicadorCarga);
      }
      else{
        //pasar peticion al proceso siguiente
        MPI_Send(&origen, 1, MPI_INT, siguiente, PETICION, comunicadorCarga);
      }
      //sondear si hay mensajes pendientes de otros procesos
      MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, comunicadorCarga, &flag, &status);
